...
type Principio = {
  ...
  motivo: string; ejemplo: string; aplica: string[]; ref: string;
};
...
export default function PrincipiosDiseno() {
  ...
  useEffect(() => {
    api.principiosDiseno().then((d) => setItems(d.items)).catch((e) => setErr(String(e)));
  }, []);
  ...
  return (
    <div className="parq">
      <div className="parq-head">
        ...
        <p>Convenciones de diseño y UI que se aplican a toda pantalla nueva. Se van sumando durante la construcción;
          cada decisión de diseño se toma a la luz de esta lista y, si aporta una convención nueva, se agrega — con su ejemplo.</p>
        ...
        {items.map((p, i) => (
          <article className="parq-card" key={p.id}>
            ...
            <div className="parq-body">
              ...
              <div className="parq-ej">
                <span className="parq-lbl">Ejemplo</span>
                <p>{p.ejemplo}</p>
              </div>
              <div className="parq-foot">
                ...
                <code className="parq-ref" title="Referencia (archivo / hallazgo)">{p.ref}</code>
              </div>
            </div>
          </article>
        ))}
      ...
      <style>{`
        ...
        .parq-card { display:flex; gap:16px; background:var(--surface); border:1px solid var(--border); border-radius:14px; padding:16px 18px; }
        .parq-num { font-size:1.4rem; font-weight:700; color:var(--brand-2); font-variant-numeric:tabular-nums; line-height:1; min-width:34px; opacity:.85; }
        ...
        .parq-cat { font-size:.66rem; text-transform:uppercase; letter-spacing:.5px; font-weight:700;
          color:var(--brand-2); background:color-mix(in srgb, var(--brand-2) 12%, transparent); border-radius:6px; padding:3px 8px; white-space:nowrap; }
        .parq-top h2 { margin:0; font-size:1.05rem; line-height:1.3; }
        .parq-enun { margin:0 0 10px; line-height:1.55; color:var(--ink); }
        .parq-porque, .parq-ej { border-left:3px solid var(--border); padding:2px 0 2px 12px; margin:0 0 10px; }
        .parq-ej { border-left-color:var(--brand-2); }
        .parq-lbl { display:block; font-size:.64rem; text-transform:uppercase; letter-spacing:.5px; color:var(--ink-faint); margin-bottom:2px; }
        .parq-porque p, .parq-ej p { margin:0; color:var(--ink-soft); line-height:1.5; font-size:.9rem; }
        .parq-foot { display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-top:4px; }
        .parq-aplica { display:flex; gap:6px; flex-wrap:wrap; }
        .parq-chip { font-size:.72rem; color:var(--ink-soft); background:var(--surface-2); border:1px solid var(--border); border-radius:999px; padding:2px 9px; }
        .parq-ref { font-size:.72rem; color:var(--ink-faint); background:var(--surface-2); border:1px solid var(--border); border-radius:6px; padding:2px 7px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100%; }
      `}</style>
    </div>
  );
}
